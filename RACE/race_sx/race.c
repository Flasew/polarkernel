...
static int race_ioctl(struct cdev *dev, u_long cmd, caddr_t data, int fflag, 
    struct thread *td){
    ...
    switch (cmd) {
        ...
        case RACE_IOC_LIST:
             uprintf("  UNIT\n");
             LIST_FOREACH(sc, &race_list, list) uprintf(" %d\n", sc->unit); 
                 break;
        default:
            error = ENOTTY;
            break; 
...
race_new(void) {
    ...
    sc->unit = unit; LIST_INSERT_HEAD(&race_list, sc, list); 
    return (sc); 
}
    ...
    switch (event) {
        case MOD_LOAD:
            ...
            race_dev = make_dev(&race_cdevsw, 0, 
                UID_ROOT, GID_WHEEL, 0600, RACE_NAME); 
