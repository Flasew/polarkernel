...
static struct cv wrt_cv;
static struct mtx wrt_mtx;
...
echo_modevent(module_t mod __unused, int event, void *arg __unused){
    ...
    switch (event) {
        case MOD_LOAD:
            ...
            device_get_children(devList[11], &devList, &devcount);
            uprintf("Count: %d\n", devcount);
            for (int i = 0; i < devcount; i++){
                device_print_prettyname(devList[i]);
            }
            ...
            device_printf(*dev, "device is this thing\n");
            res = bus_alloc_resource_any(*dev, SYS_RES_IRQ, 
                   &did, RF_ALLOCATED | RF_SHAREABLE);
            if (!res){
                 uprintf("NULL\n");
                 error = EINVAL;
                 break;
            }
            // error = bus_setup_intr(*dev, res, INTR_TYPE_TTY,
            //      NULL, wrt_intr, &kid, &cook);
            // if (error){
            //    bus_release_resource(*dev, SYS_RES_IRQ, kid, res);
            //    device_printf(*dev, "unable to register interrupt handler\n");
            //    uprintf("error = %d\n",error);
            // }
            // dev = decList[20];
            // int rid = devC_RID_dev;
        ...
        case MOD_UNLOAD:
            ...
            // bus_release_resource(*dev, SYS_RES_IRQ, did, res);
            // error = bus_teardown_intr(*dev, res, cook);
